...
lux.update = function(dt) {
    ...
    console.log(lux.timer.getFPS());
};

lux.draw = function() {
    lux.graphics.rectangle('fill', Player.x, Player.y, Player.width, Player.height, 20, 20);

    for (let i = 0; i < 8000; i++) {
        // lux.graphics.rectangle('fill', Math.random() * 800, Math.random() * 600, 10, 10);
        lux.graphics.circle('fill', Math.random() * 800, Math.random() * 600, 10);
    }
};
